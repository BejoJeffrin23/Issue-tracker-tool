...
let SignupComponent = class SignupComponent {
    constructor(router, socialAuthService, toastr, service) {
        this.router = router;
    ...
    let config = new angular_6_social_login__WEBPACK_IMPORTED_MODULE_12__["AuthServiceConfig"]([
        {
            ...
            provider: new angular_6_social_login__WEBPACK_IMPORTED_MODULE_12__["GoogleLoginProvider"]("591838160370-ht7fh6kt3amrtnqruo2673a7pir6u2ts.apps.googleusercontent.com")
        },
    ]);
    return config;
}
...
const sortOn = __webpack_require__(/*! sort-on */ "./node_modules/sort-on/index.js");

let AssignedIssueComponent = class AssignedIssueComponent {
    constructor(SocketService, Service, toastr, router) {
        this.SocketService = SocketService;
        this.Service = Service;
        this.toastr = toastr;
        this.router = router;
        ...
        this.commentnotify = () => {
            this.SocketService.commentNotify().subscribe((data) => {
                if (data.name = data.reporter) {
                    this.toastr.success(`${data.name} has commented his issue`);
                }
                else {
        ...
        this.watchnotify = () => {
            this.SocketService.watchNotify().subscribe((data) => {
                if (data.name = data.reporter) {
                    this.toastr.success(`${data.name} has added himself as watcher`);
                }
                else {
        ...
        this.issuenotify = () => {
            this.SocketService.issueNotify().subscribe((data) => {
                if (data.name = data.reporter) {
                    this.toastr.success(`${data.name} has edited his issue`);
                }
                else {
...
AssignedIssueComponent.ctorParameters = () => [
    { type: _socket_service__WEBPACK_IMPORTED_MODULE_6__["SocketService"] },
...
let CreateIssueComponent = class CreateIssueComponent {
    constructor(socket, toastr, router, http, service) {
        ...
        this.allUsers = () => {
            this.service.allUser().subscribe((apiResponse) => {
...
let EditIssueComponent = class EditIssueComponent {
    constructor(SocketService, toastr, Service, _route, router) {
        this.SocketService = SocketService;
        this.toastr = toastr;
        this.Service = Service;
        this._route = _route;
...
let HomeComponent = class HomeComponent {
    constructor(socket, appService, toastr, router) {
        this.socket = socket;
        this.appService = appService;
        this.toastr = toastr;
        this.router = router;
...
let IssueRouteGaurdService = class IssueRouteGaurdService {
    constructor(router) {
        this.router = router;
    }
    canActivate(route) {
...
IssueRouteGaurdService.ctorParameters = () => [
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_2__["Router"] }
];
...
let ReportedComponent = class ReportedComponent {
    constructor(socket, router, toastr, Service) {
        this.socket = socket;
        this.router = router;
        this.toastr = toastr;
        this.Service = Service;
...
let ViewIssueComponent = class ViewIssueComponent {
    constructor(socket, toastr, Service, _route, router) {
        this.socket = socket;
        this.toastr = toastr;
        this.Service = Service;
        this._route = _route;
    ...
    watch() {
        let name = ng2_cookies_ng2_cookies__WEBPACK_IMPORTED_MODULE_5__["Cookie"].get("userName");
        let y;
...
ViewIssueComponent.ctorParameters = () => [
    { type: _socket_service__WEBPACK_IMPORTED_MODULE_6__["SocketService"] },
    { type: ngx_toastr__WEBPACK_IMPORTED_MODULE_4__["ToastrService"] },
    { type: _issue_service__WEBPACK_IMPORTED_MODULE_2__["IssueService"] },
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_3__["ActivatedRoute"] },
